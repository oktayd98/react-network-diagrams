...
export class MapEditor extends React.Component {
   ...
   */
  buildTopology() {
    const topology = this.state.topo;
   ...
   */
  handleAddNodePosition(posx, posy, type) {
    const topo = this.cloneTopo();
    const { x, y } = this.constrain(posx, posy);
    const n = {
      id: this.makeId(),
  ...
  renderToolbar() {
    ...
    let addNodeStyle = { color: 'grey', marginLeft: 10 };
    ...
    let submitStyle = { color: 'white', marginLeft: 10 };
    ...
    return (
      <div style={toolbarStyle}>
        ...
        <button
          ...
          style={addNodeStyle}
          className="btn btn-default btn-xs"
          onClick={() => this.handleAddNode('area')}
        >
          <span className="glyphicon glyphicon-plus" aria-hidden="true" /> Area
        </button>
        <button
          type="button"
          style={addNodeStyle}
          className="btn btn-default btn-xs"
          onClick={() => this.handleAddNode('network')}
        >
        ...
        <button
          ...
          style={addNodeStyle}
          className="btn btn-default btn-xs"
          onClick={() => this.handleAddNode('server')}
        >
          <span className="glyphicon glyphicon-plus" aria-hidden="true" />{' '}
          Server
        </button>
    ...
    if (this.state.pendingAction) {
      if (this.state.pendingAction.action === 'add-node') {
        positionSelected = (posx, posy) =>
          this.handleAddNodePosition(posx, posy, this.state.pendingAction.type);
      }
      if (this.state.pendingAction.action === 'add-edge') {
